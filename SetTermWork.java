...
class SetTermWork extends JFrame implements ItemListener{
Connection con = null;
	Statement st = null;
    ResultSet rs;
	String sql;
    ...
    String[] columnNames = {"ID", "First Name", "Last Name","Semester"};
	...
	SetTermWork(){	
		...
		butAddStudent.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent ae){
				...
				student.dispose();
				new TermWork(exam,semester,studentID);
			}
		});
		...
		try {
			...
			// pst = con.prepareStatement("SELECT * from StudentTable where ID NOT IN(select ID from  FY_TermWorkTable) and yearFySyTy = 'FY'"); 
            ...
            int i = 0;	
             while (rs.next()) {
                ...
                model.addRow(new Object[]{ID,firstName, lastName,semester});
                i++;
            }
            if (i < 1) {
	...
	{	
		...
		try {
			...
			if(selectedItem.equals("First Year"))
			{
				pst = con.prepareStatement("SELECT * from StudentTable where 'FY' and studentID ="+id);
			}
			else if(selectedItem.equals("Second Year"))
			{
				pst = con.prepareStatement("SELECT * from StudentTable where 'SY' and studentID ="+id);
			}
			else if(selectedItem.equals("Third Year"))
			{
				pst = con.prepareStatement("SELECT * from StudentTable where 'TY' and studentID ="+id);
			}
            ResultSet rs = pst.executeQuery();
            int i = 0;	
            while (rs.next()) {
